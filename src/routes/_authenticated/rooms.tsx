...
function RoomsPage() {
  ...
  async function create() {
    ...
    const { data, error } = await supabase.from("study_rooms").insert({ host_id: user!.id, name, subject }).select().single();
  ...
  return (
    <div className="space-y-6">
      <header>
        ...
        <p className="text-sm text-muted-foreground">Co-study with classmates in real time.</p>
      </header>
      ...
      <div className="flex gap-2 rounded-xl border border-border/60 bg-card/60 p-3">
        ...
        <input value={subject} onChange={(e) => setSubject(e.target.value)} placeholder="Subject" className="w-40 rounded-lg border border-border bg-background px-3 py-2 text-sm" />
